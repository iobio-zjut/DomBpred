...
options:
    -seq_path=XXX.fasta
        The path of the target sequence to be decomposed.
    -msa_path=XXX.txt
        The path of the MSA file generated by jackhmmer of the target sequence.
    -ss2_path=XXX.ss2
        The path of the predicted ss2 file of the target sequence to be decomposed.
    -dist_path=XXX.npz
        The path of the predicted distance file of the target sequence to be decomposed. Its dimension is L*L*1.
    -log=XXX.txt
        The path of DomBpred result. If this parameter is missing, it will be directly output to the console.
"""
...
if __name__ == '__main__':
    ...
    seq_path = msa_path = ss2_path = dist_path = result_path = None
    for arg in sys.argv[1:]:
        if arg.startswith("-seq_path="):
            seq_path = os.path.abspath(arg[len("-seq_path="):])
        elif arg.startswith("-msa_path="):
            msa_path = os.path.abspath(arg[len("-msa_path="):])
        elif arg.startswith("-ss2_path="):
            ss2_path = os.path.abspath(arg[len("-ss2_path="):])
        elif arg.startswith("-dist_path="):
            dist_path = os.path.abspath(arg[len("-dist_path="):])
        elif arg.startswith("-log="):
            result_path = os.path.abspath(arg[len("-log="):])
        elif arg.startswith("-"):
    ...
    if result_a == 1:
        # If it's preliminarily judged to be a single-domain protein, output directly
        restrings = input_seq_name + "  1  1-" + str(input_seq_length) + ';'
        if result_path is None:
            sys.stdout.write(restrings)
        else:
            with open(result_path, 'a') as f:
                f.write(restrings)
    else:
        # If it's preliminarily judged to be a multi-domain protein, continue the following process
        DIST_file = dist_path
        SS2_file = ss2_path
